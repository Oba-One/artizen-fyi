...
export class ArtizenClient {
  ...
  async cached<T>(key: string, fallback: T, build: () => Promise<T>): Promise<T> {
  ...
  async cacheRead<T>(key: string): Promise<T | null> {
    const cached = await this.kv.get(key);
    return cached != null ? (JSON.parse(cached) as T) : null;
  }

  async cacheFetch<T>(key: string, build: () => Promise<T>): Promise<T> {
    const cached = await this.kv.get(key);
    if (cached != null) return JSON.parse(cached) as T;
    return this.cacheWrite(key, await build());
  }

  async cacheWrite<T>(key: string, value: T): Promise<T> {
  ...
  async rebuild<T>(key: string, build: () => Promise<T>): Promise<T | null> {
