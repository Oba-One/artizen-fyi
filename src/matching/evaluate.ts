...
export function baselineRankings(index: MatchIndex): Map<string, Ranking[]> {
...
): { scoring: ScoringConfig; metrics: HumanMatchMetrics } {
...
export function tuneSemanticWeights(
  ...
  constraints: Constraints = {},
): { scoring: ScoringConfig; metrics: HumanMatchMetrics } | null {
  const complete = ratings.filter((rating) => rating.semantic?.breakdown.semantic != null);
  if (complete.length === 0) return null;
  let best: { scoring: ScoringConfig; metrics: HumanMatchMetrics } | undefined;
