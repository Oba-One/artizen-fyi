...
`/match` accepts an existing project or a private, freeform description. The browser fetches a fixed, versioned public catalog and performs all ranking in a Web Worker. Freeform text and embeddings are kept in memory and are never added to a URL, persisted, or sent to a server or external service.

The catalog is served in three pieces, because no page needs all of it. `/match/core.json` carries funds, facets, scoring, and the published fund descriptions and eligibility language used by the ranker. The August 27 catalog measures 1,480,532 bytes uncompressed and 259,189 bytes gzip; `build:client` enforces a 1.6 MB raw / 300 KB gzip first-paint ceiling. `/match/projects.json` carries the project list without the large narrative fields and is fetched only when someone reaches for the picker. `/match/project/:id.json` carries one full record — narrative context included — which the browser fetches for the selected project before scoring, degrading to the compact row if that fetch fails. The combined `/match/index.json` remains available for the QA harness and calibration scripts. These are deploy-time static assets, so a request never reads or parses the combined KV document.
...
The ranker uses only official fund language — including each fund's published description and eligibility text — plus reviewed project-agnostic facets and distinctive core concepts. Clear overlap with published criteria adds a small boost; a project that shares several specific terms with an explicit exclusion takes a cautious, bounded penalty and gets a warning pointing at the shared language. Historical relationships, availability, and activity are display context only and never affect rank or score. The primary list holds twelve funds - a count that divides evenly across one, two, and three columns, so the grid never ends on a part-filled row. Every strong or good match comes first, then the closest exploratory ones to make up the number, each carrying the badge that says which it is. The full catalog remains available separately. Results describe thematic fit and published eligibility language; they are not eligibility decisions and say nothing about open applications, geography, or deadlines.
...
Each vector record carries a fingerprint of the exact text it was built from, so it cannot be scored against different wording. Model, catalog, and vector assets are generated in one `prepare-release` sequence and shipped together. The build fails if the browser bundle, catalog, compact project rows, full project narratives, fund vectors, or any project shard disagree about the release or fingerprint. If prepared semantic assets still fail at runtime, the deterministic matcher stays available, but the results are labelled **Baseline only** instead of looking like the final semantic ranking. The local AI control only appears once the browser confirms the pinned weights are actually being served, and it rechecks after a transient network failure.
...
npm run verify:prepared-parity -- public/match/index.json
```

The vector build writes `match-fund-vectors.bin` and 64 project shards, `match-project-vectors-N.bin`. The parity check chooses a current catalog project, embeds it as the browser would, and compares that ranking with its prepared vector: the winner must match, at least nine of the top ten must overlap, shared results may move at most two places, and score drift may not exceed 0.01. This allows minor ONNX batch-shape variance without pinning deployment to a mutable project name. A separate release regression verifies that Metacrisis remains on the first page for Green Goods while both live records exist. Wrangler’s custom build (`scripts/prepare-release.mjs`) runs the model check, live catalog build, vector build, parity and ranking checks, and browser build before every `wrangler deploy` and `wrangler versions upload` — including Cloudflare git-push auto-deploys, which clone a tree with no `public/`. `wrangler dev` skips the live crawl and only rebuilds the browser bundles.
...
A production deploy fetches and verifies the pinned model, builds one live Artizen catalog, and generates vectors from that exact catalog before Wrangler uploads any assets. The vector version is derived from the taxonomy version, so taxonomy changes invalidate old vectors automatically. `npm run build:client` warns when local assets are absent, fails on a mixed or incomplete release, and fails the deploy if required assets are missing.
