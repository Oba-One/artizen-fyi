...
npm run verify:prepared-parity -- public/match/index.json "Green Goods"
```

The vector build writes `match-fund-vectors.bin` and 64 project shards, `match-project-vectors-N.bin`. The parity check embeds Green Goods as the browser would and compares that ranking with the prepared project vector: the winner must match, at least nine of the top ten must overlap, shared results may move at most two places, and score drift may not exceed 0.01. This allows minor ONNX batch-shape variance without allowing the baseline-versus-semantic split that prompted the guard. Wrangler’s custom build (`scripts/prepare-release.mjs`) runs the model check, live catalog build, vector build, Green Goods parity check, and browser build before every `wrangler deploy` and `wrangler versions upload` — including Cloudflare git-push auto-deploys, which clone a tree with no `public/`. `wrangler dev` skips the live crawl and only rebuilds the browser bundles.
