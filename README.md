...
Green means one thing on a fund card: how well the fund fits. Only the fit band is filled with green - Moss for a strong fit, Algae for a good one, a wash of Algae on Moon for worth a look, a dashed Stone outline for limited evidence. Applied, Curated, and Funded are the other green on the card - white on `--green-ink`, so the mark and the type stay at 4.5:1. Applied uses a send mark rather than a check, because a tick reads as acceptance. Available money and a fund that is not curating are Stone outlines on Moon. Curating is the default, so it is not labelled.
