...
To work on the on-device AI path, fetch its assets once (see [Fund matching](#fund-matching)); without them the “Improve with local AI” control stays hidden. Wrangler snapshots static assets when it starts, so restart `npm run dev` after generating the model or vector files.
