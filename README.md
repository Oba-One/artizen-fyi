...
Green means one thing on a fund card: how well the fund fits. Strong and good meter fills, Applied, Curated, and Funded are all `--green-ink`. Worth a look is a wash of that on Moon; limited evidence is a dashed Stone outline. White type on green-ink stays at 4.5:1. Applied uses a send mark rather than a check, because a tick reads as acceptance. Available money and a fund that is not curating are Stone outlines on Moon. Curating is the default, so it is not labelled.
